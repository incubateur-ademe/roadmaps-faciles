...
export const GET = async (request: NextRequest) => {
  ...
  try {
    ...
    if (!member.isActive) {
      return NextResponse.json({ error: "Ce membre n'est plus actif sur l'Espace Membre." }, { status: 400 });
    }
    ...
    if (existingUser && existingUser.id !== payload.userId) {
      return NextResponse.json({ error: "Ce login Espace Membre est déjà lié à un autre compte." }, { status: 409 });
    }
